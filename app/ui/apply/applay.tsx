...
export default async function ApplyPage({ searchParams }: ApplyPageProps) {
  ...
  const maybeVacancyParam = Array.isArray(resolvedParams.vacancyId)
    ? resolvedParams.vacancyId[0]
    : resolvedParams.vacancyId;
  const selectedVacancy =
    vacancies.find(
      (vacancy) => String(vacancy.id) === String(maybeVacancyParam)
    ) ?? vacancies[0];
